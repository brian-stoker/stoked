...
// --- Debugging Start ---
console.log(`[Integration Config] process.env.ARTIFACT_TIMESTAMP: ${process.env.ARTIFACT_TIMESTAMP}`);
// --- Debugging End ---
...
const artifactDir = path.join(path.resolve(__dirname, '../../test/reports/artifacts/integration'), timestamp);

// --- Debugging Start ---
console.log(`[Integration Config] Using timestamp: ${timestamp}`);
console.log(`[Integration Config] Calculated artifactDir: ${artifactDir}`);
// --- Debugging End ---
...
export default defineConfig({
  test: {
    ...
    setupFiles: [path.resolve(__dirname, './integration.setup.js')],
    reporters: ['json'],
    coverage: {
      provider: 'v8',
