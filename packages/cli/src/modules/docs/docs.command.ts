  ...
  private async createPullRequest(packageNames: string[]): Promise<void> {
    try {
      ...
      if (packageNames.length === 1) {
        // Single package - use stoked/docs-${package}-${stoked-version}
        // The stoked-version indicates which version of the tool generated the docs
        branchName = `stoked/docs-${packageNames[0].replace('@', '').replace('/', '-')}-${stokedVersion}`;
      } else {
        // Multiple packages or entire repo - use stoked/docs-${stoked-version}
        branchName = `stoked/docs-${stokedVersion}`;
      }
...
Generated using Stoked v${this.getStokedVersion().replace(/-/g, '.')}${this.testMode ? ' (TEST MODE)' : ''}`;
  ...
  private async processPackage(packagePath: string, files: string[]): Promise<void> {
    ...
    const concurrencyLevel = parseInt(process.env.LLM_CONCURRENCY || '5', 10);
...
- Run 'stoked docs process-batch' when completed
